...
# Patch 41: Unlock the CustomOAI BYOK vendor on VS Code Stable (Deprecated).
#
# Upstream previously shipped `customoai` with "when": "productQualityType != 'stable'".
# We dropped the clause so it was universally visible. Now upstream deprecated `customoai`
# in favor of `customendpoint` which has no such restriction.
# We leave this patch as a no-op fallback in case older manifests are used.
#
...
# SUPERSEDED: upstream 0.55+ getCopilotToken already stores tokenBefore and
# fires fireCopilotTokenChange (→ onDidCopilotTokenChange) when the token
# changes value. ConversationFeature listens to onDidCopilotTokenChange
# (line 133) as well as onDidAuthenticationChange, so the fake-token first
# mint is picked up natively. No source edit needed.
node << 'PATCH46_EOF'
const fs = require("fs");
const f = "src/platform/authentication/common/authentication.ts";
const code = fs.readFileSync(f, "utf8");
// If the old patch is still present, remove it — it fires a redundant event.
if (code.includes("this.fireAuthenticationChange('getCopilotToken success')")) {
  const patched = code.replace(
    /const copilotTokenBefore = this\._tokenStore\.copilotToken;\n\t\t\tconst token = await this\._tokenManager\.getCopilotToken\(force\);\n\t\t\tthis\._tokenStore\.copilotToken = token;\n\t\t\tthis\._copilotTokenError = undefined;\n\t\t\tif \(copilotTokenBefore\?\.token !== token\.token.*?\) \{\n\t\t\t\tthis\.fireAuthenticationChange\('getCopilotToken success'\);\n\t\t\t\}\n\t\t\treturn token;/s,
  ...
  if (patched !== code) {
    require("fs").writeFileSync(f, patched);
    console.log("Removed stale authentication.ts Patch 46 (superseded by upstream)");
    process.exit(0);
  }
}
console.log("Patch 46 (authentication.ts): superseded by upstream getCopilotToken fireCopilotTokenChange, nothing to do");
PATCH46_EOF
...
# ── Patch 64: Set default BYOK utility models in configurationDefaults ─────────
# Injects chat.utilityModel and chat.utilitySmallModel into package.json's
# configurationDefaults, explicitly routing them to byokauto/byok-auto so
# background tasks (renames, chat titles, etc.) use the cost-aware BYOK router
# without nagging the user to configure them.
node << 'PATCH64_EOF'
const fs = require("fs");
const f = "package.json";
const pkg = JSON.parse(fs.readFileSync(f, "utf8"));
...
let changed = false;
if (pkg.contributes.configurationDefaults["chat.utilityModel"] !== "byokauto/byok-auto") {
  pkg.contributes.configurationDefaults["chat.utilityModel"] = "byokauto/byok-auto";
  changed = true;
}
if (pkg.contributes.configurationDefaults["chat.utilitySmallModel"] !== "byokauto/byok-auto") {
  pkg.contributes.configurationDefaults["chat.utilitySmallModel"] = "byokauto/byok-auto";
  changed = true;
}

if (changed) {
  fs.writeFileSync(f, JSON.stringify(pkg, null, "\t") + "\n");
  console.log("Patched: package.json configurationDefaults for BYOK Auto utilities (Patch 64)");
} else {
...
install_byok_file \
  ".github/byok-patches/files/openRouterProvider.ts" \
  "src/extension/byok/vscode-node/openRouterProvider.ts"

install_byok_file \
  ".github/byok-patches/files/openRouterEndpoint.spec.ts" \
  "src/extension/byok/vscode-node/test/openRouterEndpoint.spec.ts"

node << 'PATCH_OPENROUTER_EOF'
const fs = require("fs");
const f = "src/extension/byok/vscode-node/byokContribution.ts";
let code = fs.readFileSync(f, "utf8");

if (!code.includes("openRouterProvider")) {
  code = code.replace("import { XAIBYOKLMProvider } from './xAIProvider';", "import { XAIBYOKLMProvider } from './xAIProvider';\nimport { OpenRouterLMProvider } from './openRouterProvider';");
  code = code.replace("this._providers.set(XAIBYOKLMProvider.providerId, xai);", "this._providers.set(XAIBYOKLMProvider.providerId, xai);\n\t\tthis._providers.set(OpenRouterLMProvider.providerId, instantiationService.createInstance(OpenRouterLMProvider, this._byokStorageService));");
  fs.writeFileSync(f, code);
  console.log("Patched: byokContribution.ts (OpenRouter registration)");
}
PATCH_OPENROUTER_EOF

node << 'PATCH_FUSION_EOF'
const fs = require("fs");
const f = "src/extension/byok/vscode-node/byokContribution.ts";
let code = fs.readFileSync(f, "utf8");

if (!code.includes("byokFusionProvider")) {
  ...
  fs.writeFileSync(f, code);
  console.log("Patched: byokContribution.ts (Fusion registration)");
}
PATCH_FUSION_EOF
...
install_byok_file \
  ...
  "src/extension/byok/vscode-node/geminiInteractionProvider.ts"
